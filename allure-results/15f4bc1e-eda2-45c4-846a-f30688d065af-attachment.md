...
- generic [ref=e1]:
  ...
  - generic [ref=e12]:
    ...
    - group "Suggession Class Example" [ref=e23]:
      ...
      - textbox "Type to Select Countries" [ref=e25]: India
...
```ts
  ...
  21 | 
  22 |     expect(await page.locator('#displayed-text').isVisible()).toBeTruthy();
  23 |     await page.locator('#hide-textbox').click();
> 24 |     expect(await page.locator('#displayed-text').isVisible()).toBe(true);
     |                                                               ^ Error: expect(received).toBe(expected) // Object.is equality
  25 | 
  26 |     await page.locator("#mousehover").hover();
  27 |     await page.getByText('Reload').click();
  28 |     // await page.pause();
  29 |     const locato = page.frameLocator("#courses-iframe").getByText('Blog');
  30 |     await locato.first().click();
  31 |     await page.reload({ waitUntil: "networkidle" });
  32 |     
  33 | 
  34 | 
  35 | })
