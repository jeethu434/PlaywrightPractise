...
```
Error: locator.allTextContents: Unexpected token "/" while parsing css selector ".//td". Did you mean to CSS.escape it?
```
...
```ts
  ...
  5  |     const lsitLocators = await page.locator('//div[@class="tableFixHead"]//table[@id="product"]//tr');
  6  |     const count = await lsitLocators.count();
  7  |     for(let i=0;i<count;i++){
  8  | 
> 9  |     console.log(lsitLocators.nth(i).locator(".//td").allTextContents());
     |                                                      ^ Error: locator.allTextContents: Unexpected token "/" while parsing css selector ".//td". Did you mean to CSS.escape it?
  10 |     }
  11 | 
  12 | 
