...
```
Error: locator.click: Error: strict mode violation: getByText('Blog') resolved to 2 elements:
    1) <a href="https://rahulshettyacademy.com/blog/">Blog</a> aka getByRole('link', { name: 'Blog' })
    2) <a href="https://rahulshettyacademy.com/blog/">Blog</a> aka getByText('Blog').nth(1)

Call log:
  - waiting for locator('#courses-iframe').contentFrame().getByText('Blog')
...
```ts
  ...
  24 |     await page.pause();
  25 |     const locato = page.frameLocator("#courses-iframe").getByText('Blog');
> 26 |     await locato.click();
     |                  ^ Error: locator.click: Error: strict mode violation: getByText('Blog') resolved to 2 elements:
  27 | 
  28 | 
  29 | 
